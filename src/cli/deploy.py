...
def deploy_to_runpod(
    ...
    env_vars = {"GITHUB_BRANCH": branch}

    print("🚀 Creating RunPod instance...")
    pod = client.create_pod(template_id, env_vars, name=f"mistral-app-{branch}")
    pod_id = pod["id"]
    ...
    for _ in range(30):  # Wait up to 5 minutes
        ...
        if status in ("RUNNING", "Rented"):
            break
        time.sleep(10)
    else:
        raise TimeoutError(
            f"Pod {pod_id} did not start in time. Status: {status}"
        )
